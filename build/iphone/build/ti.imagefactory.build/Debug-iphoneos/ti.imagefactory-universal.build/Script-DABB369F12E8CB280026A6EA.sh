...
if [ "x$TITANIUM_CLI_XCODEBUILD" == "x" ]; then
    /usr/local/Cellar/node/0.10.36/bin/node "/usr/local/bin/ti" build --platform iphone --sdk "3.5.0.GA" --no-prompt --no-progress-bars --no-banner --no-colors --build-only --xcode
    exit $?
else
